// авторское решение из модуля 3 урока 2 темы 2.
...
func main() {
	http.HandleFunc("/books", func(w http.ResponseWriter, r *http.Request) {
		...
		// Проводим проверку на допустимые значения параметров.
		if page <= 0 {
		...
		// Рассчитываем индексы начала и окончания для пагинации.
		...
		if startIndex >= len(books) {
			...
			sendJSONResponse(w, booksSubset)

			return
		}

		// Если конечный индекс превышает общее количество книг, то устанавливаем его в конец.
		if endIndex > len(books) {
			endIndex = len(books)
		}

		// Выбираем подмножество книг на основе пагинации.
		booksSubset := books[startIndex:endIndex]
